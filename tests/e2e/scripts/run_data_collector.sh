...
#
# 使用方式：
#   bash scripts/test/run_data_collector.sh [options]
#
# 选项：
#   --full        运行完整测试套件
...
check_prerequisites() {
    ...
    # 检查项目根目录
    print_step "检查项目结构..."
    if [ -d "tests/e2e" ] && [ -f "tests/e2e/test_data_collector.py" ]; then
        print_success "测试文件存在"
    else
        print_error "测试文件不存在或路径错误"
        print_info "  请在 quants-infra 项目根目录运行此脚本"
        all_ok=false
    fi
...
run_tests() {
    ...
    # 构建 pytest 命令
    local pytest_cmd="python3 -m pytest tests/e2e/test_data_collector.py"
