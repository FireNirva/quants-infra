...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║          安全配置验收测试 - 基于配置文件的 CLI                        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "${BLUE}测试配置${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  测试类型: ${GREEN}安全配置完整验收测试${NC}"
echo -e "  测试文件: tests/acceptance/test_config_security.py"
...
echo -e "${BLUE}测试内容${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  ${GREEN}✓${NC} 步骤 1/13: 实例创建和SSH连接验证"
echo -e "  ${GREEN}✓${NC} 步骤 2/13: 初始安全配置 (系统更新、工具安装)"
echo -e "  ${GREEN}✓${NC} 步骤 3/13: 防火墙配置 (iptables DEFAULT DROP)"
echo -e "  ${GREEN}✓${NC} 步骤 4/13: SSH安全加固 (22→6677、密钥认证)"
echo -e "  ${GREEN}✓${NC} 步骤 5/13: fail2ban安装和配置"
echo -e "  ${GREEN}✓${NC} 步骤 6/13: 全面安全验证"
echo -e "  ${GREEN}✓${NC} 步骤 7/13: 防火墙规则持久化验证"
echo -e "  ${GREEN}✓${NC} 步骤 8/13: 监听端口验证"
echo -e "  ${GREEN}✓${NC} 步骤 9/13: 配置标记文件验证"
echo -e "  ${GREEN}✓${NC} 步骤10/13: 系统日志访问性验证"
echo -e "  ${GREEN}✓${NC} 步骤11/13: 服务专用安全配置"
echo -e "  ${GREEN}✓${NC} 步骤12/13: CLI状态命令测试"
echo -e "  ${GREEN}✓${NC} 步骤13/13: 备份文件验证"
echo ""
...
echo ""
echo -e "${YELLOW}⚠️  此测试将创建真实的 AWS Lightsail 实例并配置完整安全设置${NC}"
...
echo "测试类型: 安全配置完整验收测试" >> "$SUMMARY_FILE"
echo "测试文件: tests/acceptance/test_config_security.py" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
# 使用 tee 同时输出到终端和文件
set +e
pytest tests/acceptance/test_config_security.py \
    -v -s \
    --tb=short \
    --capture=no \
    --color=yes \
...
if [[ $TEST_EXIT_CODE -eq 0 ]]; then
    ...
    echo ""
    echo -e "  ${GREEN}✓${NC} 实例创建和SSH连接成功"
    echo -e "  ${GREEN}✓${NC} 初始安全配置完成"
    echo -e "  ${GREEN}✓${NC} 防火墙配置正确 (默认DROP策略)"
