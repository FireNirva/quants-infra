...
# 测试内容:
#   - 实例创建
#   - 监控栈部署 (Prometheus + Grafana + Alertmanager + Node Exporter)
#   - 服务健康验证
#   - 资源清理
...
SUMMARY_FILE="$LOGS_DIR/monitor_${TIMESTAMP}_summary.txt"
ERROR_FILE="$LOGS_DIR/monitor_${TIMESTAMP}_errors.txt"

echo -e "${BLUE}╔══════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║          监控系统验收测试 - 基于配置文件的 CLI                        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "${BLUE}测试配置${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  测试类型: ${GREEN}监控系统配置文件验收测试${NC}"
echo -e "  测试文件: tests/acceptance/test_config_monitor.py"
...
echo -e "${BLUE}测试内容${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "  ${GREEN}✓${NC} 步骤 1: 创建监控实例"
echo -e "  ${GREEN}✓${NC} 步骤 2: 获取实例 IP"
echo -e "  ${GREEN}✓${NC} 步骤 3: 部署监控栈组件："
echo -e "      • Prometheus (port 9090) - 指标收集和存储"
echo -e "      • Grafana (port 3000) - 可视化仪表板"
echo -e "      • Alertmanager (port 9093) - 告警处理"
echo -e "      • Node Exporter (port 9100) - 系统指标导出"
echo -e "  ${GREEN}✓${NC} 步骤 4: 验证所有服务健康状态"
...
if ! command -v ansible-playbook &> /dev/null; then
    ...
    echo -e "${YELLOW}监控部署需要 Ansible，请安装: pip install ansible${NC}"
    exit 1
fi
...
echo ""
echo -e "${CYAN}监控栈组件：${NC}"
echo -e "  • ${GREEN}Prometheus${NC}  - 时序数据库和查询引擎"
echo -e "  • ${GREEN}Grafana${NC}     - 可视化仪表板"
echo -e "  • ${GREEN}Alertmanager${NC} - 告警路由和通知"
echo -e "  • ${GREEN}Node Exporter${NC} - 系统指标收集"
echo ""
echo -e "${YELLOW}⚠️  此测试将创建真实的 AWS Lightsail 实例并部署完整监控栈${NC}"
...
echo "测试类型: 监控系统配置文件验收测试" >> "$SUMMARY_FILE"
echo "测试文件: tests/acceptance/test_config_monitor.py" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
# 使用 tee 同时输出到终端和文件
set +e
pytest tests/acceptance/test_config_monitor.py \
    -v -s \
    --tb=short \
    --capture=no \
    --color=yes \
...
if [[ $TEST_EXIT_CODE -eq 0 ]]; then
    ...
    echo ""
    echo -e "  ${GREEN}✓${NC} 监控实例创建成功"
    echo -e "  ${GREEN}✓${NC} Prometheus 部署成功 (port 9090)"
    echo -e "  ${GREEN}✓${NC} Grafana 部署成功 (port 3000)"
    echo -e "  ${GREEN}✓${NC} Alertmanager 部署成功 (port 9093)"
    echo -e "  ${GREEN}✓${NC} Node Exporter 部署成功 (port 9100)"
