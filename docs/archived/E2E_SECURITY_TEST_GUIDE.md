# 🔒 端到端安全测试指南

## 概述

本测试将在真实的 AWS Lightsail 环境中执行完整的安全配置流程，验证所有安全功能是否正常工作。

---

## ⚠️ 重要提示

### 费用说明
- **预计费用**: 约 $0.01-0.02 USD
- **实例类型**: nano_2_0 (512MB RAM, 1 vCPU)
- **测试时间**: 约 10-15 分钟
- **自动清理**: 测试完成后自动删除实例

### 前提条件
1. ✅ AWS 凭证已配置
2. ✅ 有 Lightsail 创建和删除权限
3. ✅ Conda 环境 `quants-infra` 已创建
4. ✅ 所有依赖已安装

---

## 📋 测试内容

### 测试 1: 实例创建验证
- 创建 Lightsail 测试实例
- 验证实例状态和 IP
- 测试 SSH 连接

### 测试 2: 初始安全配置
- 系统更新
- 安装基础工具 (iptables, net-tools, python3-pip)
- 创建安全目录结构

### 测试 3: 防火墙配置 (default profile)
- 配置 iptables 规则
- 设置默认 DROP 策略
- 验证基本规则（ESTABLISHED, RELATED, Loopback）

### 测试 4: SSH 安全加固
- 修改 SSH 端口 (22 → 6677)
- 禁用密码认证
- 禁用 Root 登录
- 启用密钥认证
- 验证新端口连接

### 测试 5: fail2ban 安装
- 安装 fail2ban
- 配置 sshd jail
- 验证服务运行状态

### 测试 6: 安全配置验证
- 运行完整安全验证
- 检查所有安全标记
- 验证配置文件

### 测试 7: 防火墙规则持久化
- 检查 iptables-persistent 配置
- 验证规则文件存在
- 确认规则已保存

### 测试 8: 开放端口检查
- 检查监听端口
- 验证 SSH 端口 (6677)
- 确认无不必要端口开放

### 测试 9: 安全配置标记
- 检查所有安全标记文件
- 验证配置完成状态

### 测试 10: 系统日志检查
- 检查认证日志 (/var/log/auth.log)
- 检查 fail2ban 日志

### 测试 11: 数据采集器配置 (高级)
- 调整防火墙为数据采集器配置
- 验证 VPN 限制端口

### 测试 12: CLI 命令测试 (高级)
- 测试 quants-infra security 命令

### 测试 13: 备份文件验证 (高级)
- 检查备份目录
- 验证备份文件

---

## 🚀 执行测试

### 方法 1: 使用自动化脚本 (推荐)

```bash
cd /Users/alice/Dropbox/投资/量化交易/infrastructure

# 添加执行权限
chmod +x run_e2e_security_tests.sh

# 运行测试
./run_e2e_security_tests.sh
```

脚本会：
1. ✅ 检查 AWS 凭证
2. ✅ 激活 Conda 环境
3. ✅ 确认测试执行
4. ✅ 运行所有测试
5. ✅ 生成测试报告
6. ✅ 自动清理资源

### 方法 2: 手动执行

```bash
cd /Users/alice/Dropbox/投资/量化交易/infrastructure

# 激活环境
conda activate quants-infra

# 运行测试
pytest tests/e2e/test_security_e2e.py -v -s
```

---

## 📊 测试输出

### 成功输出示例

```
=========================================
测试 1: 验证实例已创建
=========================================
✓ 实例名称: security-test-1234567890
✓ 实例 IP: 52.xxx.xxx.xxx
✓ SSH 密钥: /Users/alice/.ssh/lightsail-test-key.pem

=========================================
测试 2: 初始安全配置
=========================================
执行初始安全配置...
✓ 初始安全配置成功

验证安装的工具...
  ✓ iptables 已安装
  ✓ iptables-persistent 已安装
  ✓ net-tools 已安装
  ✓ python3-pip 已安装

=========================================
测试 3: 防火墙配置 (default)
=========================================
配置防火墙 (default profile)...
✓ 防火墙配置成功

验证防火墙规则...
  ✓ 默认策略正确 (INPUT: DROP, FORWARD: DROP, OUTPUT: ACCEPT)
  ✓ ESTABLISHED,RELATED 连接允许
  ✓ Loopback 接口允许

...
```

### 测试报告位置

测试报告自动保存到：
```
infrastructure/test_reports/e2e_security_test_YYYYMMDD_HHMMSS.log
```

---

## 🐛 故障排除

### 问题 1: AWS 凭证未配置

**错误信息**:
```
✗ AWS 凭证未配置
```

**解决方案**:
```bash
export AWS_ACCESS_KEY_ID=your_access_key
export AWS_SECRET_ACCESS_KEY=your_secret_key
export AWS_DEFAULT_REGION=ap-northeast-1
```

### 问题 2: Lightsail 密钥对不存在

**错误信息**:
```
KeyPairNotFoundException
```

**解决方案**:
测试会自动创建密钥对 `lightsail-test-key`。如果失败，手动创建：

```bash
aws lightsail create-key-pair \
  --key-pair-name lightsail-test-key \
  --query 'privateKeyBase64' \
  --output text > ~/.ssh/lightsail-test-key.pem

chmod 600 ~/.ssh/lightsail-test-key.pem
```

### 问题 3: 实例启动超时

**原因**: AWS Lightsail 有时响应较慢

**解决方案**:
- 等待更长时间（测试默认等待 3 分钟）
- 检查 AWS Lightsail 控制台状态
- 重新运行测试

### 问题 4: SSH 连接失败

**可能原因**:
1. 实例尚未完全初始化
2. 安全组配置问题
3. SSH 密钥权限问题

**解决方案**:
```bash
# 检查密钥权限
ls -la ~/.ssh/lightsail-test-key.pem
# 应该显示 -rw------- (600)

# 如果权限不对
chmod 600 ~/.ssh/lightsail-test-key.pem

# 手动测试 SSH 连接
ssh -i ~/.ssh/lightsail-test-key.pem ubuntu@<instance-ip>
```

### 问题 5: 测试中断后资源未清理

**解决方案**:
```bash
# 查找测试实例
aws lightsail get-instances --query 'instances[?starts_with(name, `security-test-`)].name' --output text

# 手动删除
aws lightsail delete-instance --instance-name security-test-XXXXXXXXXX
```

---

## 📈 测试指标

### 预期测试时间

| 阶段 | 预计时间 | 说明 |
|------|---------|------|
| 实例创建 | 2-3 分钟 | Lightsail 实例启动 |
| SSH 就绪 | 1-2 分钟 | 系统初始化 |
| 安全配置 | 3-5 分钟 | 所有安全 playbook |
| 验证测试 | 2-3 分钟 | 所有验证步骤 |
| 资源清理 | 1 分钟 | 删除实例 |
| **总计** | **10-15 分钟** | - |

### 成功率目标

- ✅ 测试通过率: 100% (13/13)
- ✅ 配置成功率: 100%
- ✅ 验证通过率: 100%

---

## 🔍 深度验证

### 手动验证清单

如果需要更深入的验证，可以在测试完成但实例未删除前，手动检查：

```bash
# SSH 连接到测试实例
ssh -p 6677 -i ~/.ssh/lightsail-test-key.pem ubuntu@<instance-ip>

# 1. 检查防火墙规则
sudo iptables -L -v -n

# 2. 检查 SSH 配置
cat /etc/ssh/sshd_config | grep -E "Port|PasswordAuthentication|PermitRootLogin|PubkeyAuthentication"

# 3. 检查 fail2ban 状态
sudo fail2ban-client status
sudo fail2ban-client status sshd

# 4. 检查安全标记
ls -la /etc/quants-security/

# 5. 检查监听端口
sudo netstat -tulnp

# 6. 检查系统日志
sudo tail -50 /var/log/auth.log
sudo tail -50 /var/log/fail2ban.log

# 7. 检查备份文件
ls -la /etc/quants-security/backups/
```

---

## 📝 测试记录模板

```markdown
# 端到端安全测试记录

**测试日期**: YYYY-MM-DD HH:MM:SS
**测试人员**: [Your Name]
**环境**: AWS Lightsail ap-northeast-1

## 测试结果

- [ ] 测试 1: 实例创建验证
- [ ] 测试 2: 初始安全配置
- [ ] 测试 3: 防火墙配置
- [ ] 测试 4: SSH 安全加固
- [ ] 测试 5: fail2ban 安装
- [ ] 测试 6: 安全配置验证
- [ ] 测试 7: 防火墙规则持久化
- [ ] 测试 8: 开放端口检查
- [ ] 测试 9: 安全配置标记
- [ ] 测试 10: 系统日志检查
- [ ] 测试 11: 数据采集器配置
- [ ] 测试 12: CLI 命令测试
- [ ] 测试 13: 备份文件验证

## 问题记录

| 测试项 | 问题描述 | 解决方案 | 状态 |
|--------|---------|---------|------|
|        |         |         |      |

## 总结

**通过率**: __/13 (___%)
**总时间**: ___ 分钟
**建议**: 

```

---

## 🎯 下一步

### 测试通过后

1. ✅ 查看详细测试报告
2. ✅ 归档测试日志
3. ✅ 更新项目文档
4. ✅ 标记生产就绪

### 测试失败时

1. 📋 记录失败详情
2. 🐛 分析失败原因
3. 🔧 修复问题
4. 🔄 重新运行测试

---

## 📞 支持

**项目维护者**: Infrastructure Team  
**文档版本**: 1.0  
**最后更新**: 2024-11-21

---

## 🎉 成功标准

测试成功的标准：

✅ 所有 13 个测试通过  
✅ 无错误日志  
✅ 所有安全标记文件存在  
✅ 防火墙规则正确配置  
✅ SSH 加固生效  
✅ fail2ban 正常运行  
✅ 测试实例自动清理  

**祝测试顺利！** 🚀

